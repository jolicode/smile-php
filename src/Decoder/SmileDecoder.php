...
class SmileDecoder
{
    ...
    private function decodeValue(int $byte): DecodingResult
    {
        return match (true) {
            ...
            $byte < Bytes::THRESHOLD_SIMPLE_LITERALS => new DecodingResult($this->getSharedValue($byte)), // 1 > 31 values are string shared values.
            ...
            $byte < Bytes::THRESHOLD_TINY_ASCII => new DecodingResult(null, true), // 43 > 63 are reserved for future use.
            $byte < Bytes::THRESHOLD_SMALL_ASCII => new DecodingResult($this->writeUnicodeOrASCII($byte, 1)), // 64 > 95 are tiny ASCII.
            $byte < Bytes::THRESHOLD_TINY_UNICODE => new DecodingResult($this->writeUnicodeOrASCII($byte, 33)), // 97 > 127 are small ASCII.
            $byte < Bytes::THRESHOLD_SMALL_UNICODE => new DecodingResult($this->writeUnicodeOrASCII($byte, 2)), // 128 > 159 are tiny Unicode.
            $byte < Bytes::THRESHOLD_SMALL_INT => new DecodingResult($this->writeUnicodeOrASCII($byte, 34)), // 160 > 191 are small Unicode.
            $byte < Bytes::THRESHOLD_LONG_ASCII => new DecodingResult($this->writeSmallInt($byte)), // 192 > 223 are small int.
            $byte < Bytes::THRESHOLD_LONG_UNICODE => new DecodingResult($this->writeLongUnicodeOrASCII()),  // 224 > 227 are long ASCII.
            $byte < Bytes::THRESHOLD_7BITS => new DecodingResult($this->writeLongUnicodeOrASCII()), // 228 > 231 are long unicode.
            $byte < Bytes::THRESHOLD_LONG_SHARED_STRING => new DecodingResult(null, true), // 232 > 235 are 7 bits encoded values.
            $byte < Bytes::THRESHOLD_HEADER_BIT_VERSION => new DecodingResult($this->getLongSharedString()), // 236 > 239 are long strings shared values.
            $byte < Bytes::THRESHOLD_STRUCTURE_LITERALS => new DecodingResult(null, true), // 240 > 247 are reserved for future use.
    ...
    private function decodeKey(int $byte): DecodingResult
    {
        return match (true) {
            ...
            Bytes::LITERAL_EMPTY_STRING === $byte => new DecodingResult(''), // 32 is empty string
            $byte < Bytes::THRESHOLD_LONG_SHARED_KEY => new DecodingResult(null, true), // 33 > 47 are reserved
            $byte < Bytes::KEY_RESERVED => new DecodingResult($this->getSharedKey((($byte & 3) << 8) | $this->getNextByte())), // 48 > 51 are long shared keys
            $byte < Bytes::KEY_FORBIDDEN_KEY => new DecodingResult(null, true), // 52 > 57 are reserved
            ...
            $byte < Bytes::KEY_SHORT_ASCII => new DecodingResult($this->getSharedKey($byte & 63)), // 64 > 127 are short shared keys
            $byte < Bytes::KEY_SHORT_UNICODE => new DecodingResult($this->writeUnicodeOrASCII($byte, 1, true)), // 128 > 191 are short ASCII
            $byte < Bytes::LITERAL_ARRAY_START => new DecodingResult($this->writeUnicodeOrASCII($byte, 2, true)), // 192 > 247 are short Unicodes
    ...
    private function decodeFloat(int $bytesAmount): string
    {
        ...
        return unpack('d', pack('Q', $result))[1];
    }
    ...
    private function writeUnicodeOrAscii(int $byte, int $bits, bool $isKey = false): string
    {
        $length = ($byte & 31) + $bits;
    ...
    private function writeLongUnicodeOrASCII(): string
    {
    ...
    {
        return $this->zigZagDecode($byte & 31);
    }
    ...
    private function getLongSharedString(): string
    {
    ...
    /** @throws UnexpectedValueException */
    private function getSharedValue(int $byte): ?string
    {
        if (!$this->context->hasSharedValues()) {
            throw new UnexpectedValueException('Trying to read a shared value but shared values are disabled.');
        }
